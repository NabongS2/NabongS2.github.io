...
- [ ] **목록 썸네일** — 커버가 없는 글은 목록에서 얇은 그라데이션 띠다. SNS 카드를 그대로 넣어봤지만(2026-09-23) 바로 위 제목을 한 번 더 보여줄 뿐이라 되돌렸다. 다시 한다면 카드에 제목 대신 시리즈·분류처럼 목록에 없는 정보를 넣는다.
- [ ] **전체 글 목록(아카이브)** — 글이 20편 가까이 되면 연도·월별로 제목과 날짜만 한 줄씩 적은 페이지를 만들고, 목록(`postCount`, 지금 20)은 최신 글만 보이게 줄인다. 플러그인의 목록 넘김은 "최신 글 / 이전 글" 버튼뿐이라 쪽 번호가 없다.
